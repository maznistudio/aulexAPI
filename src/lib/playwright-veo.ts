...
export async function generateVideoViaPlaywright(
  ...
  try {
    ...
    // Type with random delays to appear human
    for (const char of prompt) {
      await textarea.type(char, { delay: Math.random() * 50 + 20 })
    }
    await sleep(500)
